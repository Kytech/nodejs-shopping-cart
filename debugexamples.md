...
Just make sure the server-side is running under nodemon, either through a `launch.json` config (whether global nodemon or from an npm script) or by directly invoking the server in debug mode with nodemon from the terminal. Really, all that matters is that the server is running under nodemon when you're debugging.
...
If the green dot disappears from a file or does not appear and it is a file that is used as part of the page that you are viewing, the connection may not be working and might require a page reload or removing and re-adding the folder using the steps above.
...
There's much power in these debug tools. This guide contains a good amount of info, but it's just the tip of the iceberg of what can be done when combining Chrome dev tools and VSCode's debugging tools. Experiment a bit yourself. You may find something not written here that may enhance your workflow.
